...
import {
  portfolioNamesArbitrum,
  portfolioNamesMainnet,
  portfolioNamesOptimism,
  portfolioNamesPolygon,
...
const QuickTrade = (props: {
  ...
  } = useApproval(sellToken, routerAddress, sellTokenAmountInWei)
  const {
  ...
  useEffect(() => {
    ...
    if (props.eiOnly) {
      setBestOption(QuickTradeBestOption.exchangeIssuance)
    } else {
  ...
  const isNotTradable = (token: Token | undefined) => {
    if (token && chainId === MAINNET.chainId)
      return (
        indexNamesMainnet.filter((t) => t.symbol === token.symbol).length !==
          0 &&
        portfolioNamesMainnet.filter((t) => t.symbol === token.symbol)
          .length !== 0
      )
    if (token && chainId === POLYGON.chainId)
      return (
        indexNamesPolygon.filter((t) => t.symbol === token.symbol).length !==
          0 &&
        portfolioNamesPolygon.filter((t) => t.symbol === token.symbol)
          .length !== 0
      )
    if (token && chainId === OPTIMISM.chainId)
      return (
        indexNamesOptimism.filter((t) => t.symbol === token.symbol).length !==
          0 &&
        portfolioNamesOptimism.filter((t) => t.symbol === token.symbol)
          .length !== 0
      )
    if (token && chainId === ARBITRUM.chainId)
      return (
        indexNamesArbitrum.filter((t) => t.symbol === token.symbol).length !==
          0 &&
        portfolioNamesArbitrum.filter((t) => t.symbol === token.symbol)
          .length !== 0
      )
    return false
  }
