...
export function getPriceImpactColorCoding(
  ...
  if (priceImpact < -5) {
    return colors.themeCopper;
  }

  if (priceImpact < -3) {
    return colors.themeNavy;
  }
...
export function getPriceImpact(
  ...
  const priceImpact = (diff / inputTotal) * -100; //TODO: If diff == 0 the universe implodes
...
//   const networkFeeDisplay = networkFee
