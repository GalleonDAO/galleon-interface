...
export const useBalance = () => {
  ...
  const [mergeBalance, setMergeBalance] = useState<Balance>(BigNumber.from(0))
  const [spiBalance, setSpiBalance] = useState<Balance>(BigNumber.from(0))
  const [feesBalance, setFeesBalance] = useState<Balance>(BigNumber.from(0))
  const [voteBalance, setVoteBalance] = useState<Balance>(BigNumber.from(0))
  ...
  useEffect(() => {
    ...
    const fetchAllBalances = async () => {
      ...
      const mergeBalance = await balanceOf(
        MergeIndex,
        chainId,
        account,
        web3Provider,
      )
      const spiBalance = await balanceOf(
        SpartanIndex,
        chainId,
        account,
        web3Provider,
      )
      const voteBalance = await balanceOf(
        veTokenIndex,
        chainId,
        account,
        web3Provider,
      )
      const feesBalance = await balanceOf(
        CryptoFeesIndex,
        chainId,
        account,
        web3Provider,
      ...
      setMergeBalance(mergeBalance)
      setSpiBalance(spiBalance)
      setVoteBalance(voteBalance)
      setFeesBalance(feesBalance)
    ...
    (tokenSymbol: string): BigNumber | undefined => {
      switch (tokenSymbol) {
        ...
        case BasisYieldEthIndex.symbol:
          return byeBalance
        case MergeIndex.symbol:
          return mergeBalance
        case SpartanIndex.symbol:
          return spiBalance
        case veTokenIndex.symbol:
          return voteBalance
        case CryptoFeesIndex.symbol:
          return feesBalance
